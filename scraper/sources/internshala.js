...
const CATEGORY_URLS = [
  { url: 'https://internshala.com/internships/computer-science-internship', category: 'Computer Science' },
  { url: 'https://internshala.com/internships/web-development-internship', category: 'Web Development' },
  { url: 'https://internshala.com/internships/data-science-internship', category: 'Data Science' },
  { url: 'https://internshala.com/internships/machine-learning-internship', category: 'Machine Learning' },
  { url: 'https://internshala.com/internships/artificial-intelligence-ai-internship', category: 'Artificial Intelligence' },
  { url: 'https://internshala.com/internships/python-django-development-internship', category: 'Python/Django' },
  { url: 'https://internshala.com/internships/mobile-app-development-internship', category: 'Mobile Development' },
  { url: 'https://internshala.com/internships/electronics-internship', category: 'Electronics' },
  { url: 'https://internshala.com/internships/electrical-engineering-internship', category: 'Electrical Engineering' },
  { url: 'https://internshala.com/internships/mechanical-engineering-internship', category: 'Mechanical Engineering' },
  { url: 'https://internshala.com/internships/civil-engineering-internship', category: 'Civil Engineering' },
  { url: 'https://internshala.com/internships/embedded-systems-internship', category: 'Embedded Systems' },
];
...
function parseCard($, el, category) {
  ...
  // Title + Link
  const $titleLink = $card.find('h3 a, a.job-title-href, .job-internship-name a').first();
  ...
  // Company
  const company = (
    $card.find('p.company a, a.link_display_like_text, .company-name').first().text().trim() ||
  ...
  // Location
  const location = (
    $card.find('.location_link, .location a, #location_names a, .ic-16-map-pin + span').first().text().trim() ||
    'Not specified'
  );

  // Stipend
  const stipend = (
    $card.find('.stipend, .ic-16-money + span, .stipend_container_toolbar_498 span').first().text().trim() ||
    'Not disclosed'
  ).replace(/\s+/g, ' ');

  // Duration
  const duration = (
    $card.find('.ic-16-calendar + span, .other_detail_item .item_body:contains("Months"), .other_detail_item .item_body:contains("Month")').first().text().trim() ||
    ''
  );

  // Start date
  const startDate = $card.find('.start_date, .ic-16-calendar + .item_body').first().text().trim() || '';

  // Apply by / Deadline
  const applyBy = $card.find('.apply_by .item_body, .apply_by_date').first().text().trim() || '';

  // Applicants
  const applicants = $card.find('.applications_message, .desktop .applications_message').first().text().trim() || '';

  // Skills — Internshala sometimes shows required skills
  const skillsList = [];
  $card.find('.skills_container span, .skill, .round_tabs, .matching_tag').each((_, skillEl) => {
    const skillText = $(skillEl).text().trim();
    if (skillText && skillText.length < 50 && !skillsList.includes(skillText)) {
  ...
  // Detect WFH
  if (link.includes('work-from-home') || location.toLowerCase().includes('work from home')) {
...
function parseWithRegex(html, category) {
  ...
  const pattern1 = /class="job-title-href"[^>]*href="([^"]+)"[^>]*>(?:<[^>]+>\s*)*([^<]+)<\//gi;
  let match;
  while ((match = pattern1.exec(html)) !== null) {
    ...
    if (locMatch) location = locMatch[1].replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    if (link.includes('work-from-home')) location = 'Work From Home';

    // Extract company from URL
    let company = 'Unknown';
    const compMatch = link.match(/-at-([a-z0-9-]+?)(\d{5,})$/);
    if (compMatch) company = compMatch[1].replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  ...
  if (internships.length === 0) {
    const pattern2 = /<a[^>]*href="(\/internship\/detail\/[^"]+)"[^>]*>([^<]+)<\/a>/gi;
...
async function scrape() {
  ...
  for (const { url, category } of CATEGORY_URLS) {
    try {
      ...
      console.log(`  [${SOURCE}]   → ${added} new listings (${listings.length} total on page)`);

      // Rate limiting — wait between requests
      await sleep(2500);
    } catch (err) {
      console.error(`  [${SOURCE}]   ✗ Failed to scrape ${category}: ${err.message}`);
    }
...
function inferSkillsFromTitle(title, category) {
  ...
  const skillMap = {
    'Python': /python/i,
    'Java': /\bjava\b/i,
    'JavaScript': /javascript|\bjs\b|node|react|angular|vue/i,
    'C++': /c\+\+|cpp/i,
