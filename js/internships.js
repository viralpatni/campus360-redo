...
(function () {
  ...
  const categoryKeywords = {
    cs: ['computer science', 'software', 'web development', 'full stack', 'frontend', 'backend', 'python', 'java', 'javascript', 'node', 'react', 'angular', 'django', 'mobile app', 'android', 'ios', 'flutter', 'devops', 'cloud', 'cs', 'it', 'programming', 'coding', 'php', 'wordpress', 'cyber security', 'game development'],
    ece: ['electronics', 'electrical', 'embedded', 'vlsi', 'iot', 'communication', 'signal', 'circuit', 'pcb', 'microcontroller', 'arduino', 'raspberry', 'ece', 'eee', 'power', 'robotics', 'mechatronics', 'drone', 'hardware'],
    mech: ['mechanical', 'manufacturing', 'automobile', 'automotive', 'cad', 'cam', 'solidworks', 'autocad', 'thermal', 'hvac', 'robotics', 'mechatronics', '3d printing', 'prototype', 'production', 'industrial'],
    data: ['data science', 'data analyst', 'machine learning', 'artificial intelligence', 'ai', 'ml', 'deep learning', 'nlp', 'big data', 'analytics', 'statistics', 'data engineering', 'tableau', 'power bi', 'neural', 'computer vision'],
  };
      ...
      chip.addEventListener('click', () => {
        if (chip.dataset.source !== undefined) {
          ...
          if (chip.classList.contains('active')) {
            ...
            filterChips.forEach(c => {
              if (c.dataset.category !== undefined) c.classList.remove('active');
            });
            chip.classList.add('active');
  ...
  async function fetchInternships(forceRefresh = false) {
    ...
    const action = forceRefresh ? 'refresh' : 'fetch';
    const url = `${API_BASE}?action=${action}&source=all`;
    ...
    try {
      ...
      if (window.LIVE_INTERNSHIPS && window.LIVE_INTERNSHIPS.internships) {
        ...
        lastUpdated.textContent = 'Live data loaded from script: ' + date.toLocaleTimeString();
      } else {
        showError('No live data found. Please run "python scrape.py" to fetch latest internships.');
      }
      statsBar.style.display = 'flex';
      renderCards();
    } finally {
  ...
  function renderCards() {
    ...
    if (currentSource !== 'all') {
      filtered = filtered.filter(item => {
        ...
        if (currentSource === 'webscraper') return src.includes('webscraper');
        return true;
      });
    }
  ...
  function createCard(item, index) {
    ...
    const sourceClass = (item.source || '').toLowerCase().includes('internshala') ? 'internshala' : 'webscraper';
    const sourceLabel = (item.source || '').toLowerCase().includes('internshala') ? 'Internshala' : 'Webscraper';

    // Tags HTML
    const tagsHTML = (item.tags || []).map(tag =>
      `<span class="intern-tag">${escapeHTML(tag)}</span>`
    ).join('');
    ...
    card.innerHTML = `
      ...
      <div class="intern-card-footer">
        ...
        ${item.link ? `<a href="${escapeHTML(item.link)}" target="_blank" rel="noopener noreferrer" class="intern-view-btn">Apply →</a>` : ''}
      </div>
    `;
